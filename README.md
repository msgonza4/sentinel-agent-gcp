...
🏗️ Architecture & Technology StackThe system is built on a "loosely coupled" architecture to ensure scalability and high availability:  Model: Gemini 2.5 Pro (Google’s most advanced reasoning engine).  Compute: Google Cloud Run (Serverless API hosting).  Database: Cloud Firestore (Real-time, structured threat repository).  CI/CD: GitHub Actions (Automated testing and deployment pipeline).  Security: IAM Workload Identity Federation for keyless, zero-trust authentication.  

👥 The TeamMia Gonzalez (@msgonza4): Security & DevOps Lead. Responsible for the Zero-Trust security posture, IAM Identity Federation, system specifications, and documentation of the human element in cybersecurity.  Terrell Elliott (@T-Elliott01): Infrastructure & Cloud Architect. Responsible for GCP API ecosystem setup, Cloud Run orchestration, and core Python/Flask API development.  
